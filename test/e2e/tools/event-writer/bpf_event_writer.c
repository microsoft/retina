...
event_writer(xdp_md_t* ctx) {
	...
	if (flt_evttype == CILIUM_NOTIFY_TRACE) {
        ...
        bpf_perf_event_output(ctx, &cilium_events, BPF_F_CURRENT_CPU, &trc_elm, sizeof(struct trace_notify));
    }
    ...
    if (flt_evttype == CILIUM_NOTIFY_DROP) {
        ...
        bpf_perf_event_output(ctx, &cilium_events, BPF_F_CURRENT_CPU, &drp_elm, sizeof(struct drop_notify));
    }
