...
func (g *ScriptGenerator) Generate() string {
	...
	// Conditionally write probes based on config
	if g.config.EnableDrops {
	...
	if g.config.EnableErrors {
		sb.WriteString(g.generateSocketErrorTracepoint())
	}

	if g.config.EnableRetransmits {
...
/*
 * Network Issue Tracer - Generated by retina bpftrace
 * Traces:
