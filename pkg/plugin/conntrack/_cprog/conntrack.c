...
/**
    * The structure representing a key in the connection tracking map.
 **/
struct ct_key {
    __u32 src_ip;
    __u32 dst_ip;
...
/** 
    ...
    * @arg new_key The key to be reconstructed.
    * @arg key The key to be used as a reference.
**/
...
/**
    ...
    * @arg dir The direction of the packet.
    * Returns true if the packet should be report to userspace. False otherwise.
**/
static __always_inline bool ct_process_packet(struct ct_key *key, __u8 flags) {
    if (!key) {
        return false;
    }
    ...
    if (!value) { // The connection is not found in the forward direction. Check the reverse direction.
        ...
        struct ct_key reverse_key;
        __builtin_memset(&reverse_key, 0, sizeof(struct ct_key));
        ...
        if (!value) {
            ...
            if (now >= lifetime || flags & (TCP_FIN | TCP_RST)) {
                ...
                value->is_closing = 1;
                bpf_map_update_elem(&retina_conntrack_map, &reverse_key, value, BPF_ANY);
            ...
            __u8 seen_flags;
            __u64 last_report;
            seen_flags = value->flags_seen;
            last_report = value->last_report;
            // Update the flags.
            flags |= seen_flags;
            // We will report this packet if a new flag is seen or the report interval has passed.
        ...
        if (now >= lifetime || flags & (TCP_FIN | TCP_RST)) {
            // The connection is closing or closed. Mark the connection as closing.
            value->is_closing = 1;
            bpf_map_update_elem(&retina_conntrack_map, &new_key, value, BPF_ANY);
            return true; // Report the last packet received.
        }
        // Update the lifetime of the connection.
        value->lifetime = now + CT_CONNECTION_LIFETIME_TCP;
