...
func (d *dns) Init() error {
	...
	// Open raw socket on the default route interface only.
	// Using ifindex=0 (all interfaces) causes double-counting: AF_PACKET sees
	// outgoing packets on both the pod's veth and the node's eth0, but incoming
	// packets only on eth0. Binding to the default interface gives 1:1 symmetry.
	...
	d.sock, err = utils.OpenRawSocket(ifIndex)
	if err != nil {
...
func int2ip(nn uint32) net.IP {
	ip := make(net.IP, 4)
	ip[0] = byte(nn & 0xFF)         //nolint:gosec // intentional truncation to extract byte
	ip[1] = byte((nn >> 8) & 0xFF)  //nolint:gosec // intentional truncation to extract byte
	ip[2] = byte((nn >> 16) & 0xFF) //nolint:gosec // intentional truncation to extract byte
	ip[3] = byte((nn >> 24) & 0xFF) //nolint:gosec // intentional truncation to extract byte
	return ip
}
