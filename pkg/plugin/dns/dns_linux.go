...
import (
	...
	_ "github.com/microsoft/retina/pkg/plugin/dns/_cprog" // nolint
...
func (d *dns) Init() error {
	...
	if err := spec.LoadAndAssign(objs, &ebpf.CollectionOptions{
		...
		d.l.Error("Error loading eBPF programs", zap.Error(err))
	...
	if err := syscall.SetsockoptInt(d.sock, syscall.SOL_SOCKET, soAttachBPF, objs.RetinaDnsFilter.FD()); err != nil {
		d.l.Error("Error attaching DNS socket filter", zap.Error(err))
...
func (d *dns) parseDNSPayload(payload []byte, isResponse bool) (dnsName string, qTypes []string, addresses []string) {
	...
	if isResponse {
		for _, answer := range dns.Answers {
			...
			case layers.DNSTypeA:
				if answer.IP != nil {
					addresses = append(addresses, answer.IP.String())
				}
			...
			case layers.DNSTypeCNAME:
				...
				if len(answer.CNAME) > 0 {
					addresses = append(addresses, "CNAME:"+string(answer.CNAME))
				}
			}
		}
	}
