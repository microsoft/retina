...
{
    __u16 drop_type;
    // TODO Investigate why this is needed, verifier fails otherwise: invalid indirect read from stack R2 off -24+2 size 8
    __u16 pad;
    __u32 return_val;
};
...
void update_metrics_map_basic(drop_reason_t drop_type, int ret_val)
{
    struct metrics_map_value *entry, new_entry = {};
    struct metrics_map_key key = {
        .drop_type = drop_type,
    ...
    if (entry)
    {
        ...
        // TODO Measure tput with atomic above
        // This might miss packets due to race 
        entry->count += 1;
    }
    else
    {
...
{
    if (retVal < 0) {
        // TODO Lossy cast - key is unsigned but we pass signed.
        // Since error codes are negative, we *always* overflow and the metric value is meaningless.  
        update_metrics_map_basic(IPTABLE_RULE_DROP, retVal);
    }
    return 0;
}
...
int BPF_PROG(tcp_v4_connect_fexit, struct sock *sk, struct sockaddr *uaddr, int addr_len, int retVal)
{
    if (retVal == 0)
    {
        return 0;
...
int BPF_PROG(inet_csk_accept_fexit, struct sock *sk, int flags, int *err, bool kern, struct sock *retsk)
{
    if (retsk != NULL) {
        return 0;
    }
    update_metrics_map_basic(TCP_ACCEPT_BASIC, 0);
    return 0;
}
...
int BPF_PROG(nf_nat_inet_fn_fexit, void *priv, struct sk_buff *skb, const struct nf_hook_state *state, int retVal)
{
    ...
    update_metrics_map_basic(IPTABLE_NAT_DROP, retVal);
    return 0;
}
...
int BPF_PROG(nf_conntrack_confirm_fexit, struct sk_buff *skb, int retVal)
{
    ...
    update_metrics_map_basic(CONNTRACK_ADD_DROP, retVal);
    return 0;
}