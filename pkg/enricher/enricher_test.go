...
func TestEnricherStandaloneWithEndpointPresent(t *testing.T) {
	...
	go func() {
		...
		for {
			...
			require.NotNil(t, receivedFlow, "Expected flow")
			require.Equal(t, "pod1", receivedFlow.GetPodName())
			require.Equal(t, "ns1", receivedFlow.GetNamespace())
