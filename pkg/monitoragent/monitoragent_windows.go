...
var (
	...
	// ErrEventsMapAttachFailed is returned when the EVENTS_MAP callback registration fails.
	// This can happen when another process has already registered as a consumer of the EVENTS_MAP.
	ErrEventsMapAttachFailed = errors.New("failed to attach to EVENTS_MAP: another consumer may already be registered")
)
...
// AttachToEventsMap attempts to attach to the eBPF EVENTS_MAP for reading events.
// On Windows, only one consumer can attach to the EVENTS_MAP at a time.
// If another process has already registered, this function returns an error
// to make the failure visible rather than silently succeeding.
func (a *monitorAgent) AttachToEventsMap(int) error {
	// On Windows, attempting to attach to the EVENTS_MAP when another consumer
	// has already registered will fail. We need to detect this and return an error
	// rather than silently failing.
	//
	// Note: This implementation currently returns an error to indicate that
	// EVENTS_MAP attachment on Windows is not yet fully implemented or that
	// another consumer has already registered. When Windows eBPF support is
	// fully implemented, this should attempt to open and attach to the map,
	// and only return an error if the attachment actually fails.
	//
	// TODO: Implement proper Windows eBPF EVENTS_MAP attachment when
	// eBPF-for-Windows support is ready. For now, we return an error
	// to prevent silent failures.
	return ErrEventsMapAttachFailed
}
