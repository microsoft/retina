...
type ServiceReconciler struct {
	...
	mu           sync.RWMutex
	ipToService  map[string]types.NamespacedName   // Maps IP to service key (namespace/name)
...
func NewServiceReconciler(client client.Client) *ServiceReconciler {
	return &ServiceReconciler{
		Client:       client,
		ipToService:  make(map[string]types.NamespacedName),
...
func (r *ServiceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	r.logger.Debug("Reconciling Service", zap.String("service", req.NamespacedName.String()))
	...
	if err != nil {
		if k8serrors.IsNotFound(err) {
			// Service was deleted
			r.logger.Debug("Service not found, removing from cache", zap.String("service", req.NamespacedName.String()))
			r.removeServiceFromCache(req.NamespacedName)
			return ctrl.Result{}, nil
		}
		r.logger.Error("Failed to get Service", zap.Error(err), zap.String("service", req.NamespacedName.String()))
		return ctrl.Result{}, errors.Wrap(err, "failed to get Service")
	}

	// Handle service deletion
	if !service.DeletionTimestamp.IsZero() {
		r.logger.Info("Service is being deleted", zap.String("service", req.NamespacedName.String()))
		r.removeServiceFromCache(req.NamespacedName)
		return ctrl.Result{}, nil
	}
...
func (r *ServiceReconciler) Decode(ip netip.Addr) *flow.Service {
	...
	if serviceKey, exists := r.ipToService[ip.String()]; exists {
