...
func NewCiliumIdentityReconciler(client client.Client) CiliumIdentityReconcilerOut {
...
func (r *CiliumIdentityReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	r.logger.Debug("Reconciling CiliumIdentity", zap.String("identity", req.NamespacedName.String()))
	...
	if err != nil {
		if k8serrors.IsNotFound(err) {
			...
			r.logger.Debug("CiliumIdentity not found, removing from cache", zap.String("identity", req.NamespacedName.String()))
			r.removeIdentityFromCache(req.NamespacedName)
			return ctrl.Result{}, nil
		}
		r.logger.Error("Failed to get CiliumIdentity", zap.Error(err), zap.String("identity", req.NamespacedName.String()))
	...
	if !identity.DeletionTimestamp.IsZero() {
		r.logger.Info("CiliumIdentity is being deleted", zap.String("identity", req.NamespacedName.String()))
		r.removeIdentityFromCache(req.NamespacedName)
		return ctrl.Result{}, nil
	}
