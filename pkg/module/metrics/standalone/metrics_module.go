...
import (
	...
	enr "github.com/microsoft/retina/pkg/enricher"
...
var (
	m               *Module
	once            sync.Once
	requiredMetrics = metrics.DefaultStandaloneMetrics()
)
...
func (m *Module) updateMetricsContext() {
	...
	spec := (&api.MetricsSpec{}).WithMetricsContextOptions(requiredMetrics, metrics.DefaultCtxOptions(), metrics.DefaultCtxOptions())
	ctxType := metrics.LocalContext
