...
1. **Initialized (optional)**: The initial state upon CRD application. The Operator may set it to "Initialized" when the CRD is first applied. This state can be skipped for simplicity.

2. **Accepted**: After validating the CRD options, the Operator transitions the status to "Accepted" if everything is valid.

3. **Errored**: In case of validation issues, the Operator updates the status to "Errored" along with a reason for the CRD's invalidity.

4. **Warning**: The Operator may set the status to "Warning" if there are non-critical issues with the CRD.
...
```yaml
status:
  state: Initialized/Accepted/Errored/Warning
