...
- `CAP_SYS_ADMIN` is used to increase the memory lock limits, enabling the allocation of additional memory for eBPF programs and maps - `LoadAndAssign()` method in `dropreason_linux.go`
...
This plugin reads data from variable eBPF progs writing into the same eBPF map called `retina_dropreason_metrics`.
...
See *pkg/plugin/dropreason/_cprog/drop_reason.c*:
