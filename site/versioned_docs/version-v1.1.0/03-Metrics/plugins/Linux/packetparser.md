...
- `CAP_SYS_ADMIN` is used to load maps and programs into the kernel and assign them to user-defined structs - `LoadAndAssign()` method in `packetparser_linux.go`
- `CAP_NET_ADMIN` is used for the queuing discipline kernel mechanism - `getQdisc()` in `types_linux.go`
