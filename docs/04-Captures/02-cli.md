...
| `tcpdump-filter`      | string     | ""       | Raw tcpdump flags. Available tcpdump filters can be found in the [TCPDUMP MAN PAGE](https://www.tcpdump.org/manpages/tcpdump.1.html). By default, captures are performed on all network interfaces. You can use this flag to override the default behavior and capture on specific interfaces.  | Only works on Linux. Includes only tcpdump flags, for boolean expressions, please use include/exclude filters.     |
